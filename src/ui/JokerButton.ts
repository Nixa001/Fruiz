...
export class JokerButton {
  ...
  constructor(scene: Phaser.Scene, onUse: () => void) {
    ...
    this.button = UIHelpers.makeButton(scene, {
      ...
      width: this.width, height: 88 * k, label: 'ÉCHANGER • 1',
    ...
    this.label = this.button.list.find((child): child is Phaser.GameObjects.Text => child instanceof Phaser.GameObjects.Text)!;
  ...
  update(joker: JokerManager, canUse: boolean, sameFruit: boolean): void {
    ...
    this.label.setText(joker.ready ? 'ÉCHANGER • 1' : 'RECHARGE');
